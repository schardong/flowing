...
class SIREN(nn.Module):
    ...
    def __init__(
        ...
        net.append(
            nn.Sequential(
                nn.Linear(n_in_features, hidden_layer_config[0]), SineLayer(self.w0)
            )
        )
    ...
    def from_pretrained_initial_condition(self, other: OrderedDict):
        ...
        try:
            ...
            raise ValueError("Invalid state_dict provided. Convert it to v2 first")
        ...
        new_first_layer = torch.cat(
            (first_layer, torch.zeros_like(first_layer[..., 0].unsqueeze(-1))), dim=-1
        )
...
def from_pth(path, device="cpu", w0=1, ww=None):
    ...
    try:
        ...
        model, upgraded_to_v2 = from_state_dict(weights, device=device, w0=w0, ww=ww)
        if upgraded_to_v2:
...
class MLPv2(nn.Module):
    ...
    activation: function, optional.
        The activation function for input and intermediate layers. Examples are
        `torch.nn.GELU`, `torch.nn.TanH`, and `nn.Sigmoid`. Default value is
        `torch.nn.GELU`.
    ...
    def __init__(
        ...
        net.append(
            nn.Sequential(
                nn.Linear(n_in_features, hidden_layer_config[0]), activation()
            )
        )
    ...
    def reset_weights(self):
        """Resets the weights of the network using xavier with calcualted gain.

        Returns
        -------
        """
        activation_type = (self.activation).__name__.lower()
        for m in self.modules():
            if isinstance(m, nn.Linear):
                try:
                    gain = nn.init.calculate_gain(activation_type) / 100
                except ValueError:
                    gain = 1.0
